...
public class DealString {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(DealString.class);
				...
				if(strs!=null&&strs.length>0){
					ArrayList<String> list=new ArrayList<String>();
					 ...
					 if(list!=null&&list.size()>0){
						 anyone=(String[])list.toArray(new String[list.size()]);			 
					 }
				}
			} catch (Exception ex) {
				return null;
	...
	public static String strTotime0(String str,String sourFormat,String objFormat) {
		...
		java.text.SimpleDateFormat sdf1 = new java.text.SimpleDateFormat(sourFormat);
		Date date = null;
		try {
			date = sdf1.parse(str);
	...
	public static Date getDateTimethreeDate(String str) {		 
		//注意：SimpleDateFormat构造函数的样式与strDate的样式必须相符
		 java.text.SimpleDateFormat  simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
		  //必须捕获异常try
		 Date date=null;
		 try { 
			 date=simpleDateFormat.parse(str);
	...
	public static String getDateTimebh() {
		java.text.SimpleDateFormat f = new java.text.SimpleDateFormat("yyMM");
		String time = f.format(new java.util.Date());
		return time;
	}
	...
	public static String getDates() {
		...
		java.text.SimpleDateFormat f = new java.text.SimpleDateFormat(
				"yyyy/MM/dd HH:mm:ss");
		time = f.format(new java.util.Date());
		return time;
	}
	...
	public static String getDateTimezz() {
		java.text.SimpleDateFormat f = new java.text.SimpleDateFormat("yyyyMM");
		String time = f.format(new java.util.Date());
		return time;
	}
	...
	public static String jqXsdTwo(String no) {
		String str = no;
		if (checkXsd(no)) {
			str = no.substring(no.indexOf("."), no.length());
		}
		return str;
	}
	/** 截取小数点前面的数字 */
	...
	public static String getnumber(String str){
        ...
        for (int i = 0; i < str.length(); i++) {
            if (number.indexOf(str.charAt(i)) == -1) {
            	...
            	str1.append(str.substring(i,i+1));
            }
        }
